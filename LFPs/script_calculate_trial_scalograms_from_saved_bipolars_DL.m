...
% modified so all scalograms are stored; can pull out the relevant ones
% later
...
for i_rat = 33 : n_rats
    ...
    for i_session = 1 : num_sessions
        ...
        % got rid of selecting trials here - just compute all the
        % scalograms and pick out the ones we want later
        if isempty(trials)
        ...
        for i_lfptype = 1 : length(lfp_types)
            ...
            probe_type = lfp_data.probe_type;
            intan2probe_mapping = lfp_data.intan2probe_mapping;

%   bp single plot nn8x8 but simplified plots (no titles)
% ZH 7/25/2024

%Makes bipolar power plots for each channel
% axes color coded based on good/bad and titles color coded based on region

%could make flattened or not based on what lines are commented
% this simple version does not include titles and color codes the lines of
% the plots instead 
...
function plot_bipolar = bp_single_plot_nn8x8_SIMPLE(power_lfps_diff_fname, valid_sites_reordered)
...
for i_row = 1 : num_rows
    ...
    subplot(LFPs_per_shank,8,plot_num);
     ...
     % Flatten the spectrum while preserving peaks (commented out)
...
% Plot the flattened spectrum (log scale)
  %  plot_bipolar=plot(f, 10*log10(flattened_spectrum), 'LineWidth', 1.5,'Color',color);
  % non flattened
    plot_bipolar=plot(f, 10*log_spectrum, 'LineWidth', 1.5,'Color',color);
...
%anything else --> blue
 ...
 %   caption = sprintf('s:%d-%d in:%d-%d(1) %s-%s', channel_num(i_row_adj),channel_num(i_row_adj+1),intan_num(i_row_adj),intan_num(i_row_adj+1),region_top,region_bot); % Make a catch so this doesn't need to be edited every graph
 %   title(caption, 'FontSize', 9,'HorizontalAlignment', 'center', 'VerticalAlignment', 'top','Color',color);

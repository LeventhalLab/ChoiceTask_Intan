...
%%%This is believed to be correct but need confirmation RL 06/14/24
  ...
  %These are correct if needed but appears only needed in the
  %identify_bad_data.m script. This is the order of intan channels dorsal-ventral as they
  %appear on the probe starting with shank A
...
disp('Ordering LFPs to match NN8x8 organization')
elseif strcmpi(probe_type, 'ASSY156')
    ...
    %As far as I can tell these are only needed for identify_bad_data.m but
    %this is the order of intan channels as they appear on the probe
    %ordered dorsal-ventral starting with outside column of shank A
     intan_site_order_for_trials_struct = [23, 14, 16, 25, 12, 21, 8, 10, ...% Shank A.
                                22, 9, 20, 63, 2, 17, 30, 5,   ...
                                26, 13, 11, 24, 15, 28, 27, 6, ...
                                19, 29, 4, 32, 1, 31, 3, 7,    ...
                                37, 60, 45, 35, 62, 46, 34, 48,...% Shank B
                                18, 64, 43, 56, 55, 51, 42, 38,...
                                33, 47, 61, 36, 57, 59, 41, 52,...
                                50, 39, 54, 58, 44, 40, 53, 49];
     site_order = [26, 35, 33, 24, 37, 28, 41, 39, ...
...
ordered_lfp = intan_amplifier(intan_site_order, :);   
disp('Ordering LFPs to match Assy156 organization ')
elseif strcmpi(probe_type, 'ASSY236')
    ...
    intan_site_order = [1,9,21,4,26,5,20,27,...% Shank A % Verified 6/14/24
    ...
    %not sure if needed rl 06/14/24
    %intan_site_order and intan_site_order_for_trials_struct are equal 
    %both are the order of the the probes sites in intan going dorsal to
    %ventral starting with the outside column of shank A
    intan_site_order_for_trials_struct = [1,9,21,4,26,5,20,27,...% Shank A % Verified 6/14/24
        6,15,16,32,30,8,13,12,...
        11,25,17,3,23,2,10,18,...
        7,22,31,24,28,14,19,29,...
        54,46,59,37,34,51,41,49,...%shankB
        ...
        48,58,44,53,35,50,64,45,...
        42,55,38,43,60,56,52,40];
    % this is the site order on the probe itself found via cambridges probe
    % mapping
    site_order = [64, 48, 16, 61, 13, 60, 29, 12, ...
...
ordered_lfp = intan_amplifier(intan_site_order, :);
disp('Ordering LFPs to match Assy236 organization');
end

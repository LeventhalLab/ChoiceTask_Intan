...
function plot_bipolar = bp_single_plot_assy156(power_lfps_diff_fname, valid_sites_reordered)
...
if color_top == color_bot
  ...
  %  title(caption, 'FontSize', 7,'HorizontalAlignment', 'center', 'VerticalAlignment', 'top');
    ...
    caption = sprintf('assy156 s:%d-%d in:%d-%d(1) %s-%s', channel_num(i_row_adj),channel_num(i_row_adj+1),intan_num(i_row_adj),intan_num(i_row_adj+1),region_top,region_bot); % Make a catch so this doesn't need to be edited every graph

% monopolar display flattened

% ZHH
% should work like power plot display 4 but flattened (subtracted from a
% 1/f trend)

% script to run through folders and make flattened monopolar plots for
...
functions called:
probe_site_map_all
flattened_mp_single_plot_assy156 (and assy236, and nn8x8)
intan_and_channel_site_order

files used:
Monopolarpower.mat files in processed folders
    *these have only been updated through 465 20230424a (6/19/24)
        *they were updated through 493 eventually
channels_qc_final.xlsx (Excel file for color coding bad channels)

Outputs save to: 
output_folder =  fullfile(intan_choicetask_parent, 'mp_display_flattened');
...
% leave this section commented when wanting torun through all valid rats
   %i want only 236 rats to run rn (actually skip to 425)
     %  if contains(ratID, NN8x8)||contains(ratID, ASSY156) ||contains(ratID, 'R0420')
     %      continue;
      %  end
          %skipping to a nn8x8 rat rat, then skip to 327
       % if contains(ratID, ASSY236)||contains(ratID,ASSY156)||contains(ratID,'R0326')
       %      continue;
       % end 
        ...
        mono_power_plot = [session_name, '_mp_flattened.pdf'];
...
% Once plots/data seems accurate, can use this to skip over any existing files and only create new files. When making edits, comment this section out so new graphs are made.        
     ...
     %    end
        ...
        if contains(ratID, NN8x8)
            probe_type = 'NN8x8';
            plot_monopolar = flattened_mp_single_plot_nn8x8(power_lfps_fname, valid_sites_reordered);   % include info for making a title, etc. in the single_plot function
        ...
        elseif contains(ratID, ASSY156)
            probe_type = 'ASSY156';
            plot_monopolar = flattened_mp_single_plot_assy156(power_lfps_fname, valid_sites_reordered);  % include info for making a title, etc. in the single_plot function
            sgtitle(A, 'Interpreter','none'); % 'Interpreter', 'none'  --- allows the title to have an underscore instead of a subscript
            set(gcf, 'WindowState', 'maximize'); % maximizes the window so that it exports the graphics with appropriate font size
            exportgraphics(gcf, mono_power_plot);
            close;
        elseif contains(ratID, ASSY236)
            probe_type = 'ASSY236';
            plot_monopolar = flattened_mp_single_plot_assy236(power_lfps_fname, valid_sites_reordered);   % include info for making a title, etc. in the single_plot function
            sgtitle(A, 'Interpreter','none'); % 'Interpreter', 'none'  --- allows the title to have an underscore instead of a subscript
            set(gcf, 'WindowState', 'maximize'); % maximizes the window so that it exports the graphics with appropriate font size
            exportgraphics(gcf, mono_power_plot);
            close;
        ...
        % no bipolar (diff) plots here, see bipolar_display instead
    end

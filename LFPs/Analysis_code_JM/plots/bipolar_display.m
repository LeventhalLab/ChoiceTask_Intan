...
% created based on monopolar display flattened:
    % but bipolar

%!! if you don't want flattened (whitened) power spectra, go into the 3
% single probe typee functions and comment out the place that flattens the
% power spectra and in the plot line of the scrpt replace the flattened
% power with log_spectra


% script to run through folders and make bipolar plots for
% each channel pair on its own plot organized based on physical location
...
functions called:
probe_site_map_all
bp_single_plot_assy156 (and ASSY236, and NNsite) 
    (which calls getChannelRegionLabel that uses ProbeSite_Mapping_MATLAB_RL.xlsx)
...
Outputs save to: 
output_folder =  fullfile(intan_choicetask_parent, 'bp_display');
...
% is missing new rats - ZH 6/13/24
...
% sessions to ignore checked 6/20/24 ZH
...
for i_ratfolder = 1 : length(valid_rat_folders)
    ...
    % extract the ratID and session name from the LFP file
        ...
        % comment out this section when wanting to run all rats 
   %i want only 236 rats to run rn (skip to 425)
   %    if contains(ratID, NN8x8)||contains(ratID, ASSY156) ||contains(ratID, 'R0420')
   %        continue;
   %     end
          %skipping to a 236 rat rat
      %  if contains(ratID, NN8x8)||contains(ratID,ASSY156)
      %       continue;
      %  end 
        ...
        A=cell(3,1);
        A{1} = ['Subject: ' ratID ' Session: ' session_name ' Task Level: ' choiceRTdifficulty{logData.taskLevel+1}];
        A{2} = [];
        A{3} = []; 
        
        % load_channel_information 
        % Use the opts.VariableNamesRange for eat ratID to
        ...
        bp_power_plot = [session_name, '_bp_display.pdf'];
        bp_power_plot = fullfile(output_folder, bp_power_plot);
...
% Once plots/data seems accurate, can use this to skip over any existing files and only create new files. When making edits, comment this section out so new graphs are made.        
     ...
     %    end
        ...
        if contains(ratID, NN8x8)
            probe_type = 'NN8x8';
            plot_bipolar = bp_single_plot_nn8x8(power_lfps_diff_fname, valid_sites_reordered);   % include info for making a title, etc. in the single_plot function
            sgtitle(A, 'Interpreter','none','FontSize',26); % 'Interpreter', 'none'  --- allows the title to have an underscore instead of a subscript
            ...
            exportgraphics(gcf, bp_power_plot);
            close;
        elseif contains(ratID, ASSY156)
            probe_type = 'ASSY156';
            plot_bipolar = bp_single_plot_assy156(power_lfps_diff_fname, valid_sites_reordered);  % include info for making a title, etc. in the single_plot function
        ...
        elseif contains(ratID, ASSY236)
            probe_type = 'ASSY236';
            plot_bipolar = bp_single_plot_assy236(power_lfps_diff_fname, valid_sites_reordered);   % include info for making a title, etc. in the single_plot function
            sgtitle(A, 'Interpreter','none'); % 'Interpreter', 'none'  --- allows the title to have an underscore instead of a subscript
            set(gcf, 'WindowState', 'maximize'); % maximizes the window so that it exports the graphics with appropriate font size
            exportgraphics(gcf, bp_power_plot);
            close;

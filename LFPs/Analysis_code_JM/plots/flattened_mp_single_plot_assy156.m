% flattened mp single plot assy156
% ZH

% like flattened mp single plot nn8x8 but adjusted for assy 156

% flattened_mp_single_plot_nn8x8

function plot_monopolar_flattened = flattened_mp_single_plot_assy156(power_lfps_fname, valid_sites_reordered)
...
%
    ...
    caption = sprintf('ASSY156 s:%d in:%d (1) %s', site_num,intan_num(i_row),channel_region_label); % Make a catch so this doesn't need to be edited every graph

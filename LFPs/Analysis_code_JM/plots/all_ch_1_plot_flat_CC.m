...
%but with flattened plots
%{
quick info
Created by: ZH 6/20/24

Purpose: plots the monopolar power spectra FLATTENED based on 1/f shape for all channels on one plot 
...
Full path: 
X:\Neuro-Leventhal\data\ChoiceTask\GitHub\ChoiceTask_Intan\LFPs\Analysis_code_JM\plots\all_ch_1_flat_CC.m
...
% what goes in:
% Define the rat ID, session number, and probe type
ratID = 'R0425';
...
% what we want out:
    % one set of axis with the flattened monoppolar power spectra for all channels on
    % top of each other to easily identify outliers and bad channels
        % color code channels based on excel file
...
for i = 1:64
    ...
    log_spectrum = log10(monopolarpower_channel_data);
    log_f = log10(f);
    ...
    %else plot red
    if ones_and_twos(intan_number)==1
         plot(f, 10*log10(flattened_spectrum), 'Color', bad_color);
    elseif ones_and_twos(intan_number)==2
        plot(f, 10*log10(flattened_spectrum), 'Color', good_color);
    else
        plot(f, 10*log10(flattened_spectrum), 'Color','r');
    end
    hold on

    % label each channel
    % Calculate position for the text annotation
    text_y = 10*log10(flattened_spectrum(text_x));
...
xlabel('frequency');
ylabel('power');
title(sprintf('flattened monopolar power for all channels %s %s', ratID,session_number));
xlim([0,65]);
ylim([-20,20]);

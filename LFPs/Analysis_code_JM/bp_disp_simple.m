%bp disp simple
% just like bipolar display except no titles, lines are color coded
%just using to make a simplified display for my presentation
%THIS IS NOT READY TO BE RUN FOR ALL RATS
...
% currently only set up to go through R0412
for i_ratfolder =13
    
    session_processed_folders = valid_rat_folders(i_ratfolder).processed_folders;
    
    for i_sessionfolder = 3 : length(session_processed_folders)
   ...
   %     end
      ...
      %  end 
        ...
        output_folder =  fullfile(intan_choicetask_parent, 'bp_display_simple'); 
        if ~exist(output_folder, 'dir')
            mkdir(output_folder);
        end    
        
        % create filenames to hold mono- and diff-LFPs
        bp_power_plot = [session_name, '_bp_display_simple.pdf'];
...
% Once plots/data seems accurate, can use this to skip over any existing files and only create new files. When making edits, comment this section out so new graphs are made.        
     ...
     %    end
        ...
        if contains(ratID, NN8x8)
            probe_type = 'NN8x8';
            plot_bipolar = bp_single_plot_nn8x8_SIMPLE(power_lfps_diff_fname, valid_sites_reordered);   % include info for making a title, etc. in the single_plot function

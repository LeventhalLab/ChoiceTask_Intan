...
for i_rat = 1 : length(rats_with_intan_sessions)
    ...
    for i_sessionfolder = 1 : length(intan_folders)
        ...
        [~, session_log_name, ext] = fileparts(session_log);
        fprintf('read %s\n', strcat(session_log_name, ext))
        ...
        session_pf = processed_data_folder_from_rd_metadata(rd_metadata, intan_parent_directory);
        % mono_lfpfile = 
        sprintf('placeholder')
    end

...
% name2chk=extractfield(valid_rat_names,'name');
% name2chk=cellfun(@(X) X(end-4:end),name2chk);
% badfldrs=extractfield(valid_rat_names,'processed_folders');
% badfldrs()
...
%%%%%%%%%%%%%%%%Conditional removed to replace all of the bad/unverified data RL 06/14/24 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%              
       ...
       %if ~exist(diff_power_fn, 'file')
            ...
            if strcmpi(probe_type, 'nn8x8')
                ...
                disp(['Differentials completed for ', lfp_file.name])

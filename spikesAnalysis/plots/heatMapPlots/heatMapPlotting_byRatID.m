function heatMapPlotting_byRatID(regionUnits,params)
%% this plots heat maps for each rat within a given region for the given behavior fields and parameters 
...
behaviorFields=behaviorField;
    ...
    for r=1:length(unique(ratIDs))
        ratID=ratIDs{r};
        % if ~isfield(regionUnits.dataByRats,ratID)
        %     regionUnits.dataByRats.(ratID)={};
        % end
        % if ~isfield(regionUnits.dataByRats.(ratID),behaviorField)
        %     regionUnits.dataByRats.(ratID).(behaviorField)={};
        % end
        % 
        ...
        figFileName = fullfile(savePath, [treatmentToProcess '_eventHeatmaps_' behaviorField '_' region '_' subPlotTitle '_' ratID '.png']);

function [unitFRRTMTcorrelations]=unitFRvsRTMT(regionUnits, params)
% unitFRvsRTMT
%
...
for r = 1:length(ratIDs)
    ...
    for uu = 1:length(filteredUnitNames)
        ...
        for t = 1:length(trials)
            ...
            if t == 1
                FR_perTrial = nan(length(trials), nBins);
            ...
            for b = 1:nBins
                ...
                FR_perTrial(t,b) = nSpikes / binWidth; % Hz
            end
        end
        % -----------------------------
        ...
        for b = 1:nBins
            ...
            if params.plotScatter && RT_p(b)<0.05
                ...
                yVals = FR_perTrial(:,b);         % 1 x nTrials or nTrials x 1
        ...
        for b = 1:nBins
            ...
            RT_betaZ(b) = mdl.Coefficients.Estimate(2);   % slope
            RT_pZ(b)    = mdl.Coefficients.pValue(2);     % p-value for slope
            RT_r2Z(b)   = mdl.Rsquared.Ordinary;   % r^2
            if params.plotScatter && RT_pZ(b)<0.05
                % x = RT or MT per trial (seconds)
                xVals = RT_all;        % or MT_allTrials
        ...
        for b = 1:nBins
            ...
            MT_betaZ(b) = mdl.Coefficients.Estimate(2);
            MT_pZ(b)    = mdl.Coefficients.pValue(2);
            MT_r2Z(b)   = mdl.Rsquared.Ordinary;
        end
        %%
        ...
        unitFRRTMTcorrelations.(treatment).(ratID).(session).units.(uid).FR_perTrial=FR_perTrial;
        ...
        unitFRRTMTcorrelations.(treatment).(ratID).(session).units.(uid).RTcorrResults.beta = RT_beta;
        unitFRRTMTcorrelations.(treatment).(ratID).(session).units.(uid).RTcorrResults.p    = RT_p;
        unitFRRTMTcorrelations.(treatment).(ratID).(session).units.(uid).RTcorrResults.r2   = RT_r2;
        unitFRRTMTcorrelations.(treatment).(ratID).(session).units.(uid).RTcorrResults.betaZ = RT_betaZ;
        unitFRRTMTcorrelations.(treatment).(ratID).(session).units.(uid).RTcorrResults.pZ    = RT_pZ;
        unitFRRTMTcorrelations.(treatment).(ratID).(session).units.(uid).RTcorrResults.r2Z   = RT_r2Z;
        
        unitFRRTMTcorrelations.(treatment).(ratID).(session).units.(uid).MTcorrResults.beta = MT_beta;
        unitFRRTMTcorrelations.(treatment).(ratID).(session).units.(uid).MTcorrResults.p    = MT_p;
        unitFRRTMTcorrelations.(treatment).(ratID).(session).units.(uid).MTcorrResults.r2   = MT_r2;
        unitFRRTMTcorrelations.(treatment).(ratID).(session).units.(uid).MTcorrResults.betaZ = MT_betaZ;
        unitFRRTMTcorrelations.(treatment).(ratID).(session).units.(uid).MTcorrResults.pZ    = MT_pZ;
        unitFRRTMTcorrelations.(treatment).(ratID).(session).units.(uid).MTcorrResults.r2Z   = MT_r2Z;

        ...
        for i_session = 1 : num_sessions
            ...
            if ~isempty(trials)
                ...
                %event names that dont exist 
                    ...
                    %% Calculate psth and rasters from all good units based on trial feature events and store them in a mat file based on region
                    if ~isempty(cluster_spikes)
                        ...
                        for g = 1:size(cluster_spikes, 1)
                              ...
                              if any(strcmp(clusRegion,regionOfinterest))
                                  ...
                                  % regionSummaryPath=fullfile(parent_directory,'RegionalSummary');
                                  % regionSummaryPath=fullfile(regionSummaryPath,clusRegion);
                                  % if ~isfolder(regionSummaryPath)
                                  %   mkdir(regionSummaryPath)
                                  % end
                                  % interesting=true;
                                  % sessionName=strrep(sessionName,' ','_');
                                  
                                  % regionFile = fullfile(regionSummaryPath, [clusRegion '_unitSummary.mat']);
                                  % if isfile(regionFile)
                                  %     load(regionFile);
                                  %     if ~isfield(regionUnits,unitID)
                                  %regionUnits.(unitID)=struct();
                             ...
                             unitEphysProperties=rawEphysData.ephysProperties(g,:);
                             ...
                             if ~isfield(regionUnits.(unitID),'unitMetrics')
                                 ...
                                 regionUnits.(unitID).unitMetrics.treatement=ratTreatment;
                                 regionUnits.(unitID).unitMetrics.qMetrics=unitQmetrics;
                                 regionUnits.(unitID).unitMetrics.meanFiringRate=meanFR;
                                 regionUnits.(unitID).unitMetrics.stdMeanFR=stdMeanFR;
                                 regionUnits.(unitID).unitMetrics.unitACG=unitACG;
                                 regionUnits.(unitID).unitMetrics.templateWaveforms=spikeStruct.templateWaveforms(g,:,:);
                                 regionUnits.(unitID).unitMetrics.clusterSpikes=cluster_spikes(g,:);
                                 regionUnits.(unitID).unitMetrics.rawUnitWaveformsPath=rawUnitWaveformsPath;
                                 regionUnits.(unitID).unitMetrics.ephysProperties=unitEphysProperties;
                             end
                             if ~isfield(regionUnits.(unitID).unitMetrics,'ephysProperties')
                                regionUnits.(unitID).unitMetrics.ephysProperties=unitEphysProperties;
                             end
                             %%%
                             ...
                             if ~any(~isfield(regionUnits.(unitID).behavioralFeatures,trialfeatureList))
                                disp('this unit has already been processed continuing')
                                continue
                             end
                             for i=1:length(trialfeatureList)
                                    
                                    trialfeatures=trialfeatureList{i};
                                    featureName=strjoin(trialfeatures,'_');
                                   
                                    if ~isfield(regionUnits.(unitID).behavioralFeatures, featureName)
                                        regionUnits.(unitID).behavioralFeatures.(featureName)=struct();
                                    end
                             ...
                             %plot full acg if plotACG
                             %            nexttile(t, [1 round(numCols/2,0)]); % Span 1 row and all columns
                             %            lags = -floor(size(unitACG{1,:},2)/2) : floor(size(unitACG{1,:},2)/2);
                             %            bar(lags, unitACG{1,:}, 1); % Full-width bars
                             %            xlabel('Lag (ms)');
                             %            ylabel('ACG Count');
                             ...
                             % end
                                ...
                                % windowSizeMs = 50; % Half window size (±50 ms)
                                % 
                             ...
                             % saveas(fig, fullfile(savePath,[RAT_SESSION_UNITNAME, '.fig']))

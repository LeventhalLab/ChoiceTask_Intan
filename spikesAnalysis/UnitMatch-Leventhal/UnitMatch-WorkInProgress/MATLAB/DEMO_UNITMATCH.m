...
GithubDir = 'C:\Users\rylash\Documents\GitHub'; % Github directory
...
%% THIS IS WHERE RL MADE MODIFICATIONS Set up parent directories and ignore rat ids not interested in
...
ratID = 'R0572';
    ...
    %% find session dirs
    if exist(fullfile(rat_folder,'MatchingUnits2'))
        keyboard
    end
    UMparam.SaveDir = fullfile(rat_folder,'MatchingUnits2'); 
    ...
    UMparam.KSDir = {};
    phys_sessions={};
    %loop to add all ephys sessions from rat to umparm 
    for i_session = 1 : num_sessions
        ...
        UMparam.KSDir{i_session} = fullfile(phys_folder, 'kilosort4');
    end
% remove empty cells
UMparam.KSDir = UMparam.KSDir(~cellfun('isempty', UMparam.KSDir));
...
if UMparam.MakePlotsOfPairs
    ...
    if UMparam.GUI
        FigureFlick(UMparam.SaveDir,'RL')
        pause
    end
end

...
[~,MatchTable]=UMassessment2(MatchTable); %% RL Added this function to assist with indexing, basically ignore self matches since those are obvious
match = guiData.match;
...
%% Begin RL Modifications (8/11/25); This is the shotty fix for the orignial code lines in 205-206 which didnt correctly output curation values

tmpMatchIdx = arrayfun(@(X)find(((MatchTable.UID1 == X) | (MatchTable.UID2 == X)) & ...
         (MatchTable.MatchIdx==1)), guiData.pairIDs, 'UniformOutput', false);
outputCol = MatchTable.(guiData.name); % Get existing column

for i = 1:numel(tmpMatchIdx)
    if match(i) == 1
        outputCol(tmpMatchIdx{i}) = 1;
    end
end

MatchTable.(guiData.name) = outputCol; % Save back to table

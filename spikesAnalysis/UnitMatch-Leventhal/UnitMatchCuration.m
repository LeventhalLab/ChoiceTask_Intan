...
% [~,MatchTable]=UMassessment2(MatchTable);
...
if UMparam.MakePlotsOfPairs
   % DrawPairsUnitMatch(UMparam.SaveDir); %% If figure flick throws an error that says "images not drawn" run this line for now commented out
    if UMparam.GUI
        FigureFlick(UMSaveDir) % modified figureflick.m to load UnitMatch2.mat (line 35/193)
        pause
    end
end
